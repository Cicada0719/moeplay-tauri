...
5b. **提交推送前必须本地跑完整 CI 检查套件**（否则 PR 会被 CI 打回，浪费一轮）：`npm run verify:commands`（Tauri 命令契约）、`cargo fmt --check`、`cargo clippy --all-targets -- -D warnings`（注意是 all-targets，含测试代码）、`npm run check`。新增 Rust 命令时必须同步：build.rs 的 generate_handler 清单、capabilities 权限、permissions/autogenerated 权限文件。
