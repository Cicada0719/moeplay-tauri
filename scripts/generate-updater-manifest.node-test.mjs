...
import {
  ...
  findUpdaterArtifact,
  prepareCanonicalUpdaterArtifact,
} from "./generate-updater-manifest.mjs";

test("creates a latest.json entry from a signed canonical NSIS artifact", () => {
...
test("copies localized bundle names to the stable GitHub release asset contract", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "moeplay-updater-localized-"));
  const localized = path.join(dir, "localized MoeGame_1.2.3_x64-setup.exe");
  fs.writeFileSync(localized, "installer");
  fs.writeFileSync(`${localized}.sig`, "signature");
  const canonical = prepareCanonicalUpdaterArtifact(dir, "1.2.3");
