...
//   - 失败/无匹配不投递——条目保持文字卡；请求异常（网络/接口错误）计入
//     failedCount()，供界面给出「封面加载失败」提示（无匹配图不算失败）。
...
export interface SearchCoverFetcher {
  ...
  /** 因请求异常（网络/接口错误）未补到封面的 key 数；「无匹配图」不算失败 */
  failedCount(): number;
  clear(): void;
}
...
export function createSearchCoverFetcher(): SearchCoverFetcher {
  ...
  const failed = new Set<string>(); // 请求异常未补到图的 key（无匹配图不计）
