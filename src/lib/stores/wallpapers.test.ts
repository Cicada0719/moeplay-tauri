...
describe("wallpaperStore content rating", () => {
  it("keeps installed general wallpapers in hide mode and permits suggestive/adult wallpapers for blur or show", async () => {
    ...
    wallpaperApi.listWallpapers.mockResolvedValue([
      ...
      record({ id: "other-pack", themePack: "after-school", installed: true }),
      record({ id: "not-local", installed: true, ...{} }),
    ]);

    await store.load();

    expect(store.installedFor("yozakura", "hide").map((item) => item.asset.id)).toEqual(["general", "not-local"]);
    expect(store.installedFor("yozakura", "blur").map((item) => item.asset.id)).toEqual([
      "general",
      "suggestive",
      "adult",
      "not-local",
    ]);
    expect(store.installedFor("yozakura", "show").map((item) => item.asset.id)).toEqual([
      "general",
      "suggestive",
      "adult",
      "not-local",
...
describe("wallpaperStore background sync fallback", () => {
  it("retains already loaded wallpapers and exits syncing quietly when the manifest is unavailable", async () => {
    const store = await loadStore();
    const installed = record({ id: "builtin:yozakura:1", installed: true });
    ...
    expect(store.installedFor("yozakura").map((item) => item.asset.id)).toEqual([installed.asset.id]);
  });
  ...
  it("continues after individual download failures and never adds failed downloads to the installed pool", async () => {
    ...
    expect(store.syncing).toBe(false);
    expect(store.installedFor("yozakura", "blur")).toEqual([]);
  });
